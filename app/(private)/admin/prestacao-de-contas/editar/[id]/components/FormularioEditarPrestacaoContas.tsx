...
import {
    atualizarPrestacaoContasAdmin,
    buscarPrestacaoContasPorId,
    removerPrestacaoContasAdmin,
} from "../../../services/prestacao-contas-admin-service";
import type {
    DadosPrestacaoContasAdmin,
    MovimentacaoPrestacaoContas,
    TipoCadastroPrestacao,
...
type FormularioEditarPrestacaoContasProps = {
    idPrestacao: string;
};
...
export default function FormularioEditarPrestacaoContas({
    idPrestacao,
}: FormularioEditarPrestacaoContasProps) {
    const router = useRouter();

    const [tipoCadastro, setTipoCadastro] =
        useState<TipoCadastroPrestacao>("pdf");
    ...
    const [carregandoPrestacao, setCarregandoPrestacao] = useState(true);
    const [modalRemocaoAberta, setModalRemocaoAberta] = useState(false);

    const totaisManual = useMemo(() => {
    ...
    useEffect(() => {
        async function carregarPrestacao() {
            const resposta = await buscarPrestacaoContasPorId(idPrestacao);

            if (!resposta.sucesso || !resposta.prestacao) {
                setMensagemErro("Prestação de contas não encontrada.");
                setCarregandoPrestacao(false);
                return;
            }

            const prestacao = resposta.prestacao;

            const movimentacoesSalvas = Array.isArray(prestacao.movimentacoes)
                ? prestacao.movimentacoes
                : [];

            setTipoCadastro(prestacao.tipoCadastro ?? "pdf");
            setTitulo(prestacao.titulo ?? "");
            setMesReferencia(prestacao.mesReferencia ?? "");
            setAnoReferencia(prestacao.anoReferencia ?? "");
            setDescricao(prestacao.descricao ?? "");
            setValorRecebido(prestacao.valorRecebido ?? "0");
            setValorGasto(prestacao.valorGasto ?? "0");
            setSaldoFinal(prestacao.saldoFinal ?? "0");
            setMovimentacoes(
                movimentacoesSalvas.length > 0
                    ? movimentacoesSalvas
                    : [criarMovimentacaoVazia()]
            );
            setDocumentoBase64(prestacao.documentoBase64 ?? "");
            setNomeDocumento(prestacao.nomeDocumento ?? "");
            setDataCadastro(prestacao.dataCadastro ?? new Date().toISOString());
    ...
    function removerMovimentacao(id: string) {
        setMovimentacoes((movimentacoesAtuais) => {
            ...
            return movimentacoesAtuais.filter(
    ...
    function baixarDocumento() {
        if (!documentoBase64) {
            setMensagemErro("Nenhum PDF foi anexado nesta prestação.");
            return;
        }

        const linkDownload = document.createElement("a");

        linkDownload.href = documentoBase64;
        linkDownload.download = nomeDocumento || "prestacao-contas.pdf";
    ...
    function validarFormulario() {
        ...
        if (tipoCadastro === "pdf" && !documentoBase64.trim()) {
            return "Anexe o documento PDF.";
        }

        if (tipoCadastro === "manual") {
    ...
    async function confirmarRemocaoPrestacao() {
        setMensagemErro("");
        setMensagemSucesso("");
        ...
        try {
            ...
            const resposta = await removerPrestacaoContasAdmin(idPrestacao);
            ...
            setMensagemSucesso("Prestação de contas removida com sucesso.");
            ...
            setMensagemErro("Erro ao remover a prestação de contas.");
        } finally {
            setCarregando(false);
        }
    }
    ...
    async function enviarFormulario(event: FormEvent<HTMLFormElement>) {
        ...
        const valorRecebidoFinal =
            tipoCadastro === "manual"
                ? String(totaisManual.totalEntradas)
                : valorRecebido;

        const valorGastoFinal =
            tipoCadastro === "manual" ? String(totaisManual.totalSaidas) : valorGasto;

        const saldoFinalCalculado =
            tipoCadastro === "manual"
                ? String(totaisManual.saldoFinal)
                : saldoFinal;
        ...
        const dadosPrestacao: DadosPrestacaoContasAdmin = {
            ...
            valorGasto: valorGastoFinal,
            saldoFinal: saldoFinalCalculado,
            movimentacoes: tipoCadastro === "manual" ? movimentacoes : [],
            documentoBase64,
            nomeDocumento,
        ...
        try {
            ...
            const resposta = await atualizarPrestacaoContasAdmin(
                idPrestacao,
                dadosPrestacao
            );
            ...
            setMensagemSucesso("Prestação de contas atualizada com sucesso.");

            setTimeout(() => {
                router.push("/admin/prestacao-de-contas");
            }, 600);
        } catch {
            setMensagemErro("Erro ao atualizar a prestação de contas.");
        } finally {
            setCarregando(false);
        }
    }

    if (carregandoPrestacao) {
        return (
            <main className="flex min-h-screen items-center justify-center border-[3px] border-[#202020] bg-[#fceefd] fundo-forminhas-admin px-4 py-8">
                <div className="rounded-[8px] bg-white px-8 py-6 text-[14px] text-[#252525] shadow-[6px_6px_0_#b75fc1]">
    ...
    function gerarRelatorioManual() {
        const dadosPrestacaoAtual: DadosPrestacaoContasAdmin = {
            id: idPrestacao,
            tipoCadastro,
            titulo,
            mesReferencia,
            ...
            valorGasto:
                tipoCadastro === "manual" ? String(totaisManual.totalSaidas) : valorGasto,
            saldoFinal:
                tipoCadastro === "manual" ? String(totaisManual.saldoFinal) : saldoFinal,
            movimentacoes: tipoCadastro === "manual" ? movimentacoes : [],
            documentoBase64,
            nomeDocumento,
            dataCadastro: dataCadastro || new Date().toISOString(),
        ...
        abrirRelatorioPrestacao(dadosPrestacaoAtual);
    }

    return (
        <main className="relative flex min-h-screen items-center justify-center border-[3px] border-[#202020] bg-[#fceefd] fundo-forminhas-admin px-4 py-8">
                ...
                >
                    <div className="mb-[22px] flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="mt-[2px] text-[12px] font-light text-[#777]">
                                Altere os dados, substitua o PDF ou atualize as movimentações.
                            </p>
                        </div>
                    </div>

                    <div className="mb-5 rounded-[10px] border border-[#e5b5eb] bg-[#fdf4ff] px-4 py-3">
                        <p className="text-[12px] font-light leading-[1.5] text-[#7e3c88]">
                            Esta prestação foi cadastrada como{" "}
                            <strong>{tipoCadastro === "manual" ? "manual" : "PDF"}</strong>.
                            Para manter a organização, o tipo de cadastro não é alterado nesta
                            tela.
                        </p>
                    </div>
                        ...
                        <div className="grid grid-cols-[220px_150px_1fr] gap-[18px] max-md:grid-cols-1">
                            ...
                            <div className="rounded-[8px] border border-[#e5b5eb] bg-[#fdf4ff] px-4 py-3">
                                <p className="text-[12px] font-light leading-[1.4] text-[#7e3c88]">
                                    Referência usada para organizar o histórico de relatórios.
                                </p>
                            </div>
                        </div>
                        ...
                        {tipoCadastro === "pdf" && (
                            <div className="rounded-[10px] border border-[#b75fc1] bg-[#fdf4ff] px-4 py-4">
                                ...
                                <p className="mt-1 text-[12px] font-light text-[#777]">
                                    Você pode baixar o PDF atual ou substituir por outro arquivo.
                                </p>

                                <div className="mt-4 grid grid-cols-[1fr_150px] gap-3 max-sm:grid-cols-1">
                                    <label
                                        htmlFor="documento"
                                        className="flex h-[42px] cursor-pointer items-center rounded-[8px] border border-[#b75fc1] bg-white px-4 text-[13px] font-light text-[#777] shadow-[1px_2px_3px_rgba(0,0,0,0.10)] transition hover:bg-[#fceefd]"
                                    >
                                    ...
                                    <button
                                        ...
                                        className="h-[42px] rounded-[8px] border border-[#b75fc1] bg-white px-4 text-[13px] font-bold uppercase text-[#b75fc1] shadow-[1px_2px_3px_rgba(0,0,0,0.10)] transition hover:bg-[#fceefd]"
                                    >
                                        Baixar PDF
                                    </button>
                                </div>
                                ...
                                <div className="rounded-[10px] border border-[#b75fc1] bg-[#fdf4ff] px-4 py-4">
                                    <div className="mb-4 flex items-center justify-between gap-3 max-sm:flex-col max-sm:items-start">
                                        <div>
                                            ...
                                            <p className="mt-1 text-[12px] font-light text-[#777]">
                                                Atualize entradas e saídas da prestação.
                                            </p>
                                        </div>
                                            ...
                                            >
                                                <div className="mb-3 flex items-center justify-between">
                                                    ...
                                                    <button
                                                        type="button"
                                                        onClick={() =>
                                                            removerMovimentacao(movimentacao.id)
                                                        }
                                                        ...
                                                        <select
                                                            ...
                                                            onChange={(event) =>
                                                                atualizarMovimentacao(
                                                                    ...
                                                                    "tipo",
                                                                    event.target
                                                                        .value as TipoMovimentacaoPrestacao
                                                                )
                                                            }
                                ...
                                <div className="grid grid-cols-3 gap-4 max-md:grid-cols-1">
                                    <div className="rounded-[10px] border border-[#e5b5eb] bg-[#fdf4ff] px-4 py-4 text-center">
                                        ...
                                        <strong className="mt-1 block text-[20px] text-[#252525]">
                                            {formatarMoeda(totaisManual.totalEntradas)}
                                        </strong>
                                    </div>
                                    ...
                                    <div className="rounded-[10px] border border-[#e5b5eb] bg-[#fdf4ff] px-4 py-4 text-center">
                                        ...
                                        <strong className="mt-1 block text-[20px] text-[#252525]">
                                            {formatarMoeda(totaisManual.totalSaidas)}
                                        </strong>
                                    </div>
                                    ...
                                    <div className="rounded-[10px] border border-[#b75fc1] bg-white px-4 py-4 text-center">
                                        ...
                                        <strong className="mt-1 block text-[20px] text-[#b75fc1]">
                                            {formatarMoeda(totaisManual.saldoFinal)}
                                        </strong>
                                    </div>
                                </div>
                            </>
                    ...
                    <div className="mt-auto flex justify-between gap-[12px] pt-[22px] max-sm:flex-col">
                        <button
                            ...
                            disabled={carregando}
                            className="h-[36px] w-[150px] rounded-[7px] border border-[#b75fc1] bg-white text-[15px] font-bold uppercase text-[#b75fc1] shadow-[1px_2px_3px_rgba(0,0,0,0.16)] transition hover:bg-[#fceefd] disabled:cursor-not-allowed disabled:opacity-70 focus:outline-none focus:ring-2 focus:ring-[#b75fc1]/50 max-sm:w-full"
                        >
                            Remover
                        </button>
                        ...
                        <div className="flex justify-end gap-[12px] max-sm:flex-col">
                            {tipoCadastro === "manual" && (
                                <button
                                    ...
                                    disabled={carregando}
                                    className="h-[36px] w-[170px] rounded-[7px] border border-[#b75fc1] bg-white text-[13px] font-bold uppercase text-[#b75fc1] shadow-[1px_2px_3px_rgba(0,0,0,0.16)] transition hover:bg-[#fceefd] disabled:cursor-not-allowed disabled:opacity-70 focus:outline-none focus:ring-2 focus:ring-[#b75fc1]/50 max-sm:w-full"
                                >
                                    Gerar relatório
                                </button>
                            )}
            ...
            {modalRemocaoAberta && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/35 px-4">
                    <div className="w-full max-w-[430px] rounded-[10px] border border-[#b75fc1] bg-white px-6 py-6 shadow-[4px_5px_10px_rgba(0,0,0,0.25)]">
                        ...
                        <p className="mt-4 text-center text-[14px] font-light leading-[1.5] text-[#252525]">
                            Tem certeza que deseja remover esta prestação de contas do
                            histórico?
                        </p>
                        ...
                        <div className="mt-6 flex justify-center gap-3 max-sm:flex-col">
                            <button
                                ...
                                disabled={carregando}
                                className="h-[34px] w-[140px] rounded-[7px] border border-[#b75fc1] bg-white text-[14px] font-bold uppercase text-[#b75fc1] shadow-[1px_2px_3px_rgba(0,0,0,0.14)] transition hover:bg-[#fceefd] disabled:opacity-70 max-sm:w-full"
                            >
                                Voltar
                            </button>
                            ...
                            <button
                                type="button"
                                onClick={confirmarRemocaoPrestacao}
                                disabled={carregando}
                                className="h-[34px] w-[140px] rounded-[7px] bg-[#b75fc1] text-[14px] font-bold uppercase text-white shadow-[1px_2px_3px_rgba(0,0,0,0.20)] transition hover:bg-[#a94fb4] disabled:cursor-not-allowed disabled:opacity-70 max-sm:w-full"
                            >
